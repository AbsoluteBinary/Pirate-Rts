            ...
            if (expandGridConfiguration) {
                ...
                if (tgs.terrain != null) {
                    if (tgs.terrain.supportsMultipleObjects) {
                        EditorGUI.indentLevel++;
                        tgs.terrainObjectsPrefix = EditorGUILayout.TextField(new GUIContent("Objects gameName Prefix", "Use terrain gameobjects which has this prefix in their names (letter case ignored)."), tgs.terrainObjectsPrefix);
