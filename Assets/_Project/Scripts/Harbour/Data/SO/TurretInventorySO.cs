...
{
    [CreateAssetMenu(fileName = "TurretInventory", menuName = "Harbour/Turret Inventory")]
    public class TurretInventorySO : ScriptableObject
    {
        public List<TurretEntry> turrets = new List<TurretEntry>();
        ...
        {
            if (index < 0 || index >= turrets.Count) return false;
            if (turrets[index].count <= 0) return false;
        ...
        {
            return (index >= 0 && index < turrets.Count) ? turrets[index].count : 0;
        }
        ...
        {
            if (index < 0 || index >= turrets.Count) return;
            turrets[index].count += amount;
            OnCountChanged?.Invoke(index);
