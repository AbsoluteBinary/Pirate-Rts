...
namespace _Project.Scripts.Harbour.ShipBuilder
{
    public class ComponentSelectionHUD : MonoBehaviour
    {
        ...
        public void OpenComponentSelection(ModuleSlot targetSlot)
        {
            ...
            {
                Debug.LogError("ComponentSelectionHUD: Missing ModuleDatabase.");
                return;
            }

            _targetSlot = targetSlot;
            CloseComponentSelectionKeepSlot();

            _componentPanel = new VisualElement { name = "ComponentSelectionOverlay" };
            _componentPanel.style.position = Position.Absolute;
            _componentPanel.style.top = 0;
            _componentPanel.style.left = 0;
            _componentPanel.style.right = 0;
            _componentPanel.style.bottom = 0;
            _componentPanel.pickingMode = PickingMode.Position;
            ...
            dimmer.RegisterCallback<ClickEvent>(_ => CloseComponentSelection());
            _componentPanel.Add(dimmer);

            var card = new VisualElement { name = "ComponentSelectionCard" };
        ...
        private void ShowComponents(ComponentCategory category, string tabName)
        {
            _activeTab = tabName;
            if (_tabRow != null)
            {
            ...
            _gridHost.Clear();
            List<ComponentData> list = moduleDatabase.GetFilteredComponents(category);
            if (list == null || list.Count == 0)
            {
                var empty = new Label("No components in this tab");
            ...
            {
                if (c == null) continue;
                _gridHost.Add(CreateComponentTile(c));
            }
        }

        private VisualElement CreateComponentTile(ComponentData component)
        {
            ...
            cell.style.borderTopColor = GetRarityColor(component.rarity);
            ...
            {
                imgBtn.style.backgroundImage = new StyleBackground(component.icon);
            ...
            imgBtn.clicked += () => EquipComponentToSlot(component);
            cell.Add(imgBtn);
        ...
        private void CloseComponentSelectionKeepSlot()
        {
            if (_componentPanel == null) return;
